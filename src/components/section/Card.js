...
const catdTitle = {
  title: "해파리가 흘러갑니다 - ଳ",
  desc: "흔들흔들 , 사람들이 일상에 치이는 것처럼 해파리도 물살에 치여 움직인답니다.",
};
const cardInfo = [
  {
    cardImage: (
  ...
  },
  {
    ...
    cardTitle: "조그마한 해파리",
    cardDesc:
      " 바쁜듯 열심히 구완을 흩날리고 있지만 사실은 해파리는 흘러가고 있답니다. 하루에 할일이 지나다니는 것 뿐이라니 부러워요 저도 요 조그마한 해파리 처럼 가만히 즐기고 싶다.",
    btnLink: "/",
    btn: "자세히 보기",
    svg: (
      <svg
  ...
  },
  {
    cardImage: (
      <img
        ...
        alt="고양이 사진이 너무 잘찍혔을 때"
      />
    ),
    cardTitle: "풍선같은 해파리",
    cardDesc:
      " 보자마자 풍선같은 생김새에 웃음이 나와요. 후 바람을 부르면 하늘로 올라같 것 같습니다. 사실은 바다가 제 2의 하늘이 아닐까요? 높이 높이 올라가렴 작은 풍선 해파리야",
    btnLink: "/",
    btn: "자세히 보기",
    svg: (
      <svg
...
const CardUse = ({ cardImage, cardTitle, cardDesc, btnLink, btn, svg }) => {
  return (
    <article className="card">
      <figure className="card_header">{cardImage}</figure>
...
function Card({ attr }) {
  return (
    <section
      id="cardType"
      className={`card_wrap ${attr[0]} ${attr[1]} ${attr[2]} `}
    >
      <h2>{catdTitle.title}</h2>
      <p>{catdTitle.desc}</p>
      ...
      <div className={`card_inner ${attr[3]}`}>
        ...
        {/* <article className="card">
          ...
          <div className="card_body">
            ...
            <p className="desc">
              넙적한 상산이 작은 촉수들과 어울려 바다에서 흔들립니다. 파란색
              해파리를 멍하게 바라보다 보면 어느샌가 시간이 엄청 지나가있어요,
              그렇지만 빠져들게 되지 않나요?
            </p>
